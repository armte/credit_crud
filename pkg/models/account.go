...
type Account struct {
	...
	Account_Type string `gorm:"check:Account_Type in ('Checking','Savings','Loan')" json:"account_type"`
	Account_State string `json:"account_state"`
	Account_Postal_Code string `json:"account_postal_code"`
	Account_Country_Code string `json:"account_country_type"`
	Customer_Number int64 `gorm:"not null" json:"customer_number"`
	Card []Card `gorm:"foreignKey:Account_Number;references:Account_Number;constraint:OnUpdate:CASCADE,onDelete:CASCADE;" json:"-"`
...
func GetAccountById(Id int64) (*Account, *gorm.DB, error){
	...
	db := db.Where("Account_Number=?", Id).Find(&getAccount)
	var err error = nil
